//! Output types shared by the compose / reply / forward template
//! builders: the final [`Template`] (content + cursor), the
//! [`TemplateCursor`] used by TUI consumers to position the cursor
//! after the editor opens, and the intermediate [`TemplateBody`]
//! buffer that the builders write into.
